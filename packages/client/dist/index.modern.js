...
class Session extends useSingleton.Singleton {
  ...
  async load() {
    ...
    return this._change(async function () {
      _this._auth = await _this.readAuth();
  ...
  getToken() {
    ...
    return (_this$_auth = this._auth) == null ? void 0 : _this$_auth.token;
  }
  ...
  refresh() {
    ...
    return this._change(async function () {
      const data = await SessionUser.refresh(_this3._auth);
      const {
        user,
        auth
      } = data;

      _this3.logger.debug('Session refreshed', {
        user
      });

      await _this3.writeAuth(auth);
      return {
        user
      };
    });
...
class FirebaseSession extends Session {
  ...
  async load() {
    ...
    this.unsubscribe = this.auth.onIdTokenChanged(async function (firebase_user) {
